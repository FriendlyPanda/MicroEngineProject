...
GraphicsEngine::GraphicsEngine(FBO * fbo_ptr, MessageBoard * config_ptr, double * deltaTime) {
    ...
    this->windowWidth = stringToIntSafe(config->get("window.width").c_str(), DEFAULT_SCREEN_WIDTH);
    this->windowHeight = stringToIntSafe(config->get("window.height").c_str(), DEFAULT_SCREEN_HEIGHT);
...
int GraphicsEngine::init_() {
    ...
    auto glew_success = glewInit();
    if (glew_success != GLEW_OK) { // initialise glew and check if it initialised correctly
        log.errorMsg("glew.failed_init");
        std::cout << glew_success << std::endl;
        return -1;
    }
...
int GraphicsEngine::_run() {
    ...
    *deltaTime = currentFrame - lastFrame;
    ...
    const double desiredFPS = stringToIntSafe(config->get("window.config.max_framerate").c_str(), 30);
    ...
    glfwSwapInterval(stringToIntSafe(config->get("window.config.vsync").c_str(), 1));
